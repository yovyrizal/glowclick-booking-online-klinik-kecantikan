...
if ($conn) {
  ...
  $total_user = $r ? $r->fetch_assoc()['n'] : 0;
  $r = $conn->query("SELECT COUNT(*) as n FROM booking");
  $total_booking = $r ? $r->fetch_assoc()['n'] : 0;
  $r = $conn->query("SELECT COUNT(*) as n FROM dokter WHERE is_aktif=1");
  $total_dokter = $r ? $r->fetch_assoc()['n'] : 0;
  $r = $conn->query("SELECT COUNT(*) as n FROM testimoni");
  $total_testimoni = $r ? $r->fetch_assoc()['n'] : 0;
  $r = $conn->query("SELECT COUNT(*) as n FROM booking WHERE status='pending'");
  $booking_pending = $r ? $r->fetch_assoc()['n'] : 0;
  $r = $conn->query("SELECT COUNT(*) as n FROM booking WHERE tanggal=CURDATE()");
  $booking_hari_ini = $r ? $r->fetch_assoc()['n'] : 0;
...
<div class="stat-cards">
  ...
  <div class="stat-card">
    ...
    <div class="stat-info">
      <div class="num"><?= $total_booking ?></div>
  ...
  <div class="stat-card">
    ...
    <div class="stat-info">
      <div class="num"><?= $total_dokter ?></div>
  ...
  <div class="stat-card">
    ...
    <div class="stat-info">
      <div class="num"><?= $total_testimoni ?></div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.1rem;margin-bottom:1.6rem;">
  <div class="stat-card" style="border-color:#fde08d;">
    ...
    <div class="stat-info">
      <div class="num" style="color:#f39c12;"><?= $booking_pending ?></div>
      <div class="lbl">Booking Pending</div>
    </div>
  </div>
  <div class="stat-card" style="border-color:#a9dfbf;">
    <div class="stat-icon" style="background:#eafaf1;color:#1e8449;border-color:#a9dfbf;"><i class="fa-solid fa-calendar-day"></i></div>
    <div class="stat-info">
      <div class="num" style="color:#1e8449;"><?= $booking_hari_ini ?></div>
...
<div class="card">
  <div class="card-header">
    <h3>Booking <em>Terbaru</em></h3><a href="booking.php" class="btn btn-outline btn-sm">Lihat Semua <i class="fa-solid fa-arrow-right fa-xs"></i></a>
  </div>
    ...
    <table>
      ...
      <tbody>
        <?php if (!empty($booking_terbaru) && $booking_terbaru->num_rows > 0): while ($b = $booking_terbaru->fetch_assoc()): ?>
          ...
          <tr>
            <td colspan="8">
              <div class="empty-state"><i class="fa-regular fa-calendar-xmark"></i>
